...
contract HexToysMultipleFixed is OwnableUpgradeable, ERC1155HolderUpgradeable, Signature {
    ...
    function multipleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _amount, uint256 _price) public {
        ...
        require(nft.isApprovedForAll(_msgSender(), address(this)), "Not approve nft to staker address");
    ...
    function multipleBuy(uint256 _id, uint256 _amount, bytes calldata _signature, uint256[] memory _royaltyArray, address[] memory _receiverArray) external payable {
        require(pairs[_id].bValid, "invalid Pair id");
        ...
        IMultipleNFT(pairs[_id].collection).safeTransferFrom(item.owner, msg.sender, item.tokenId, _amount, "buy from Marketplace");
